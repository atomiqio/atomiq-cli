...
var map = new Map([['new', { description: 'Create a new atomiq app', action: create }], ['up', { description: 'Run app in a container', action: runContainer }], ['test', { description: 'Run app tests in a container', action: testContainer }], ['debug', { description: 'Debug app running in a container', action: debugContainer }], ['url', { description: 'Get URL (IP:PORT) for running app', action: url }]]);
...
function create(options) {
  ...
  _prompt2.default.get(['appname'], function (err, result) {
    ...
    try {
      ...
      console.log('[%s] Try running the app (use `up` to run in a container). Enter:\n%s\n%s\n%s or %s', _chalk2.default.green.bold('OK'), _chalk2.default.bold('   cd ' + context.name), _chalk2.default.bold('   atomiq make build'), _chalk2.default.bold('   atomiq up'));
    } catch (err) {
      console.log('[%s] %s', _chalk2.default.red('error'), err.message);
      process.exit(1);
    }
