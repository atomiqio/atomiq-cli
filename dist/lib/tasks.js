...
process.env.PATH = path.join(path.join('./node_modules', '.bin') + path.delimiter + process.env.PATH);
...
function copyDir(source, dest, options, filter) {
  mkdir('-p', dest);
  var files = fs.readdirSync(source).filter(filter);
  files.forEach(function (f) {
    var srcpath = path.join(source, f);
    var stats = fs.statSync(srcpath);
    if (stats.isFile()) {
      var destpath = path.join(dest, f);
      cp(srcpath, destpath);
...
target.monitor = function () {
  if (options.local) {
    debugReminder();
    spawn('nodemon', ['--legacy-watch', '--watch', 'dist', '-e', 'js,json', '--exec', 'node ' + app], spawnopts);
  } else {
    console.log('remember: while monitoring, you can also enter "rs" in the console to manually restart');
    spawn('nodemon', ['--legacy-watch', '--watch', 'dist', '-e', 'js,json', '--exec', 'docker-compose up --force-recreate'], spawnopts);
  }
};
...
target.host = function () {
  try {
    ...
    var port = exec('docker-compose port web 443').toString('utf8').split(':')[1].trim();
